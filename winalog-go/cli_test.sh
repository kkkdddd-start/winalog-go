...
test_command "collect_basic" "collect" "Basic forensic collection"
test_command "collect_output" "collect -o $OUTPUT_DIR/forensic_collect.zip" "Collection with output"
test_command "collect_compress" "collect --compress-level 9" "High compression"
test_command "collect_workers" "collect --workers 8" "Multi-thread collection"
test_command "collect_sysinfo" "collect --include-system-info" "Collect system info"
test_command "collect_registry" "collect --include-registry" "Collect registry persistence"
test_command "collect_tasks" "collect --include-tasks" "Collect scheduled tasks"
test_command "collect_prefetch" "collect --include-prefetch" "Collect Prefetch files"
...
test_command "rules_list_enabled" "rules list --enabled" "List enabled rules"
test_command "rules_status" "rules status" "Show rules status"
test_command "rules_status_detail" "rules status BruteForce" "Show specific rule status"
test_command "rules_validate" "rules validate" "Validate rules file"
test_command "rules_disable" "rules disable BruteForce" "Disable rule"
test_command "rules_enable" "rules enable BruteForce" "Enable rule"
