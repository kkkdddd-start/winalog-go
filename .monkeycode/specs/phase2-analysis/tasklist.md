...
- [ ] 4.1 实现分析器接口 (`analyzer.go`)
  - 定义 `Analyzer` 接口
  - 定义 `Result` 结构

- [ ] 4.2 实现暴力破解检测 (`brute_force.go`)
...
- [ ] 4.4 实现 Kerberos 分析 (`kerberos.go`)
...
- [ ] 4.5 实现 PowerShell 分析 (`powershell.go`)
